...
function Setup1Password(scope: Construct) {
  const command = 'curl -sSfo op.zip "https://cache.agilebits.com/dist/1P/op2/pkg/$(curl https://app-updates.agilebits.com/check/1/0/CLI2/en/2.0.0/N -s | grep -Eo \'[0-9]+\.[0-9]+\.[0-9]+\')/op_linux_amd64_v$(curl https://app-updates.agilebits.com/check/1/0/CLI2/en/2.0.0/N -s | grep -Eo \'[0-9]+\.[0-9]+\.[0-9]+\')".zip && unzip -od /usr/local/bin/ op.zip && rm op.zip'
